...
type ColumnFilter = "ALL" | string; // pakai column.id atau "ALL"
...
export default function BoardDetail() {
  ...
  useEffect(() => {
          ...
          col.id === task.columnId
            ? {
                ...
                tasks: col.tasks.some((t) => t.id === task.id)
                  ? col.tasks
                  : [...col.tasks, task].sort((a, b) => a.order - b.order),
              }
            : col,
        ),
      );
    ...
    socket.on("task:updated", (task: Task) => {
      ...
      // Update modal jika task yang sedang dibuka adalah task yang diupdate
      setSelectedTask((prev) =>
        prev?.id === task.id ? { ...prev, ...task } : prev,
      );
    });

    socket.on("task:deleted", (payload: { taskId: string } | string) => {
      // Handle both { taskId } object and raw string id
      ...
      setDetailOpen((prev) => {
        if (selectedTask?.id === taskId) return false;
        return prev;
      });
    });
          ...
          prev.map((col) => ({
            ...
            tasks: col.tasks
              .map((t) => {
                ...
                return u ? { ...t, order: u.order, columnId: u.columnId } : t;
              })
              .sort((a, b) => a.order - b.order),
          })),
        );
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    dragSourceColumnIdRef.current = col?.id ?? null;
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    const { active, over } = event;
    const sourceColId = dragSourceColumnIdRef.current;
      ...
      <div className="bg-[#0f0f18] border-b border-slate-800/60 px-4 sm:px-6 py-3">
        ...
        {showFilters && (
          <div className="max-w-screen-xl mx-auto mt-3 pt-3 border-t border-slate-800/60 space-y-3">
            {/* Row 1: Column tabs (To Do / Doing / Done) */}
          ...
          <div className="flex gap-4 h-full items-start min-w-max pb-6">
            {processedColumns.map((col) => (
              <ColumnDropZone
                ...
                column={col}
                onDetail={(task: Task) => {
                  // Pastikan task yang di-pass punya id yang benar
                  console.debug("[BoardDetail] open detail task:", task);
