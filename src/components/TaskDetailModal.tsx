...
export default function TaskDetailModal({ task, onClose, onUpdated }: any) {
  ...
  const [currentPriority, setCurrentPriority] = useState<Priority>(task?.priority ?? "MEDIUM");
        ...
        <div className="p-6 space-y-5">
          {isEdit ? (
            <>
              ...
              {/* Priority (edit mode) */}
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">Prioritas</label>
            ...
            <>
              {/* Judul */}
              <div>
                <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-1.5">Judul</p>
                <p className="text-base font-semibold text-white">{task.title}</p>
              </div>

              {/* Deskripsi */}
              <div>
                <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-1.5">Deskripsi</p>
                <p className="text-sm text-slate-400 leading-relaxed whitespace-pre-wrap">
                  {task.description || <span className="text-slate-600 italic">Tidak ada deskripsi</span>}
                </p>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Prioritas</p>
                  {priorityLoading && (
                    <span className="text-[10px] text-slate-600 animate-pulse">Menyimpan...</span>
                  )}
                </div>
                <div className="grid grid-cols-4 gap-2">
                  {PRIORITY_OPTIONS.map((opt) => (
                ...
                <p className="text-[11px] text-slate-700 mt-1.5">Klik untuk ubah prioritas langsung</p>
              </div>

              {/* Meta */}
              <div className="pt-2 border-t border-slate-800 flex items-center justify-between">
                <p className="text-xs text-slate-700 font-mono truncate">ID: {taskId}</p>
                {task.dueDate && (
