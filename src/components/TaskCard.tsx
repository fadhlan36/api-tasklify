  ...
  if (isDragging) {
    return (
      <div className="bg-slate-800 border border-violet-500/50 rounded-xl p-3 shadow-2xl rotate-1 scale-[1.02] opacity-95 pointer-events-none ring-1 ring-violet-500/20">
        <p className="text-[13px] font-medium text-white line-clamp-2">{task.title}</p>
        <div className="flex items-center gap-1.5 mt-2">
          <span className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded border ${priority.class}`}>
    ...
    <div
      ...
      className={`group relative bg-[#13131f] hover:bg-[#16162a] border rounded-xl p-3 transition-all duration-150 ${
        isSortableDragging
          ...
          : "border-slate-800/60 hover:border-slate-700/60 shadow-sm hover:shadow-md cursor-grab active:cursor-grabbing touch-none"
      }`}
    >
      <div className="flex items-start gap-2">
        {/* Drag indicator */}
        <div
          className="mt-0.5 flex-shrink-0 text-slate-700 group-hover:text-slate-500 transition-colors"
          aria-hidden="true"
        >
